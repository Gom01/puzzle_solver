...
def parse_image(image_path, puzzle, window=False):
    tools = Tools()
    new_width = 500
    margin_scale = 1.8
    scale_factor = 5.0
    ...
    # Draw all contours on a copy of the original resized image
    if window:
        contour_vis = image.copy()
        cv.drawContours(contour_vis, contours, -1, (0, 255, 0), 2)  # Green contours
        cv.imshow("All Detected Piece Contours", contour_vis)
        cv.waitKey(0)
        cv.destroyWindow("All Detected Piece Contours")
