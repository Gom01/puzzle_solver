...
im_gray = tools.convert_to_grayscale(image, window=True, window_size=WINDOW_SIZE, time=0)
...
for contour_piece in contours_filtered:
    ...
    piece_image_gray = tools.convert_to_grayscale(piece_image, window=False, window_size=WINDOW_SIZE_PIECE, time=000)


   # Appliquer un seuillage pour rendre l'image binaire
    _, binary_image = cv.threshold(piece_image_gray, 127, 255, cv.THRESH_BINARY_INV)

    # Appliquer la détection des bords avec Canny
    edges = cv.Canny(binary_image, 100, 200)

    # Optionnel : Appliquer une érosion ou dilatation pour affiner l'image
    kernel = np.ones((5, 5), np.uint8)
    edges = cv.dilate(edges, kernel, iterations=1)  # Dilatation pour renforcer les bords

    # Trouver les contours de la pièce de puzzle
    contours, _ = cv.findContours(edges, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)

    # Vous pouvez dessiner les contours pour visualiser
    cv.drawContours(piece_image, contours, -1, (0, 255, 0), 3)

    # Afficher l'image résultante avec les bords détectés
    cv.imshow("Contours", piece_image)
    cv.waitKey(0)
    cv.destroyAllWindows()

    contours_piece = contours
